...
public class WorkoutServiceImpl implements WorkoutService {
	...
	public WorkoutDateResponse getWorkoutDates() {
		...
		List<String> dates = sessions.stream().map(session -> session.getDate().toString())
	...
	public ApiResponse addCardioWorkout(CardioEntryDto cardioEntryDto) {
		...
		LocalDate date = cardioEntryDto.getDate() != null ? cardioEntryDto.getDate() : LocalDate.now();

		if (date.isAfter(LocalDate.now())) {
			throw new WorkoutServiceExceptions(ErrorMessages.INVALID_DATE.getMessage());
		}

		WorkoutSessionEntity session = sessionRepository.findByUserAndDate(user, date).orElseGet(() -> {
	...
	public WorkoutDayResponse getAllWorkoutForDate(String date) {
		...
		List<GroupedExerciseEntryResponse> strengthEntries = sessionEntity.getExercises().stream().map(entry -> {
		...
		// Add Cardio entries
		List<CardioEntryResponse> cardioEntries = sessionEntity.getCardioEntries().stream().map(entry -> {
